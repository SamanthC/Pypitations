...
def exploration():
    ...
    st.markdown(" - 3. Recherche de l'ensemble des maxima locaux caractérisés par les coordonnés de passage par zéro de la dérivée première du signal.")
    st.markdown(" - 4. Recherche de l'ensemble des pics R en appliquant un seuil de 0.9 sur la valeur normalisée des maxima.")
    ...
    st.markdown("Toutes les observations ont été définies sur une plage de 187 colonnes de DataFrame.") 
 
    # La base de données MITBIH, généralités   
    st.subheader("La base de données MITBIH :")         
    ...
    st.markdown("Observons d’abord la répartition des classes du dataset MIT-BIH associé aux différentes valeurs de la cible")        
    
    st.image("streamlit/images/image38.png", width = 1000, caption = "Répartition des classes dans le dataset MIT-BIH")

    st.markdown("Nous observons un déséquilibre de classe que nous devrons traiter par rééchantillonage au moment de l'entraînement des modèles.")
    ...
    st.markdown("Observons maintenant quelques exemples de signaux liés à différentes classes du dataset MIT-BIH.")
    st.image("streamlit/images/image41.png", width = 1000, caption = "Exemples de signaux choisis aléatoirement de différentes classes du dataset MIT-BIH")

    
    # PTBDB               
    st.subheader("La base de données PTB-DB :")
    ...
    st.markdown("Observons d’abord la répartition des classes du dataset PTB-DB associé aux différentes valeurs de la cible :") 
    st.image("streamlit/images/image45.png", width = 1000, caption = "Répartition des classes dans le dataset PTB-DB")
        
     

    # Exemple de signaux du dataset PTBDB
    st.markdown("Quelques signaux du dataset PTB-DB:")
    st.image("streamlit/images/image100.png", width = 1000, caption = "Quelques signaux du dataset PTB-DB")
